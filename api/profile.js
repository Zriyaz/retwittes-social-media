...
router.get("/:username", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const profileFollowStatus = await FollowerModel.findOne({ user: user._id });
    return res.json({
      profile,
      followersLength:
        profileFollowStatus.followers.length > 0
          ? profileFollowStatus.followers.length
          : 0,
      followingLength:
        profileFollowStatus.following.length > 0
          ? profileFollowStatus.following.length
          : 0,
    });
  } catch (error) {
    console.error(error);
